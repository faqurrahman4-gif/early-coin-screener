...
def run_token_pipeline(token_config: dict) -> dict:
    ...
    # PENTING: kalau kita tidak punya data kompetitor sekategori (category
    # belum diisi manual), JANGAN panggil rumus dengan angka fallback —
    # rumusnya secara matematis menghasilkan skor MAKSIMAL saat
    # total_competitors=1, padahal itu artinya "kita tidak tahu", bukan
    # "token ini dominan". Default ke 0 + tandai Insufficient, konsisten
    # dengan faktor lain yang datanya belum lengkap.
    category = token_config.get("category")
    if category and market.get("market_cap_rank"):
        ...
        f3 = {"skor": 0, "breakdown": {"catatan": "Data rank/kompetitor sekategori belum ada — isi 'category' di MANUAL_OVERRIDES"}}
    ...
    # PENTING: sama seperti Faktor 3 — kalau kita tidak punya data wallet
    # aktif ASLI (bukan placeholder 0), JANGAN panggil rumus dengan angka
    # dummy. Rumusnya menafsirkan "volume_wallet_ratio=0" sebagai "tidak ada
    # wash trading" dan "tidak ada data puncak insentif" sebagai "retensi
    # 100%" — dua-duanya salah kaprah kalau sebenarnya kita cuma belum
    # punya datanya. Default ke 0 + Insufficient, bukan 20/20.
    # TODO: sesuaikan — ambil volume_24h & wallet aktif ASLI dari block
    # explorer API (Etherscan/Solscan/dst) untuk mengisi ini dengan benar.
    if token_config.get("active_wallets_peak_incentive") is None:
        missing_manual.append("onchain_activity")
        f7 = {"skor": 0, "breakdown": {"catatan": "Data wallet aktif asli belum ada — isi 'active_wallets_peak_incentive' & volume 24h asli di MANUAL_OVERRIDES"}}
    else:
        f7 = scoring.score_onchain_activity(
            volume_wallet_ratio=token_config.get("volume_wallet_ratio_actual", 0),
            category_avg_ratio=token_config.get("category_avg_volume_wallet_ratio", 1),
            active_wallets_now=token_config.get("active_wallets_now", 0),
